...
func Build(g *Grammar) (*Table, error) {
	primaryStart := g.GetPrimaryStart()
	if primaryStart == "" {
...
func buildFollow(g *Grammar, prods []Production, augStart string) map[string]map[string]bool {
	...
	primaryStart := g.GetPrimaryStart()
	if follow[primaryStart] == nil {
		follow[primaryStart] = make(map[string]bool)
	}
	follow[primaryStart][EOF] = true
